        ...
        <div className="max-w-7xl mx-auto w-full">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
            <div>
              ...
              <p className="text-sm font-bold text-slate-400 uppercase tracking-widest mt-1">Manage rates and material standards</p>
            </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <button className="p-3.5 bg-slate-50 text-slate-400 rounded-2xl border border-slate-100 hover:bg-indigo-50 hover:text-indigo-600 transition-all active:scale-95">
          ...
          <div className="flex flex-wrap items-center gap-4 border-t border-slate-50 pt-6">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Pricing Filters:</span>
            <button className="px-5 py-2.5 bg-slate-50 text-slate-500 rounded-xl text-xs font-black uppercase tracking-widest border border-slate-100 hover:bg-indigo-50 hover:text-indigo-600 hover:border-indigo-100 transition-all">Latest Rates</button>
            <div className="px-5 py-2.5 bg-slate-50 text-slate-500 rounded-xl text-xs font-black uppercase tracking-widest border border-slate-100 flex items-center gap-2 cursor-pointer hover:bg-slate-100 group transition-all">
              Tax Category <ChevronDown className="w-4 h-4 text-slate-300 group-hover:text-indigo-600" />
            </div>
            <div className="px-5 py-2.5 bg-indigo-600 text-white rounded-xl text-xs font-black uppercase tracking-widest flex items-center gap-2 cursor-pointer shadow-lg shadow-indigo-200 active:scale-95 transition-all ml-auto md:ml-0">
            ...
            <button 
              onClick={onAddNew}
              className="md:hidden flex-1 flex items-center justify-center gap-3 py-3 bg-indigo-600 text-white rounded-xl font-black text-xs uppercase tracking-widest shadow-lg active:scale-95"
            >
      ...
      <div className="max-w-7xl mx-auto w-full p-8 lg:p-12">
        {loading ? (
          ...
          <div className="text-center py-40 bg-white rounded-[3rem] border-2 border-dashed border-slate-100 shadow-inner">
            ...
            <h3 className="text-xl font-bold text-slate-800 mb-2">No Products Configured</h3>
            <p className="text-sm text-slate-400 font-medium">Start by adding your first material rate</p>
          </div>
        ) : (
              ...
              >
                <div className="flex flex-col h-full">
                  ...
                  <div className="mt-auto pt-6 border-t border-slate-50 flex items-center justify-between">
                    <div>
                      <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest mb-1">Standard Unit</p>
                      <div className="flex items-baseline gap-1">
                        <span className="text-xl font-black text-slate-800">MTON</span>
                      </div>
                    </div>
                  ...
                  <div className="absolute top-8 right-8 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-all translate-x-4 group-hover:translate-x-0">
                    <button 
                      ...
                      className="w-10 h-10 bg-white shadow-xl rounded-xl flex items-center justify-center text-indigo-600 hover:bg-indigo-600 hover:text-white transition-all border border-indigo-50"
                    >
                      <Pencil className="w-4 h-4" />
                    </button>
                    <button 
                      onClick={(e) => { e.stopPropagation(); handleDelete(investor.id); }}
                      className="w-10 h-10 bg-white shadow-xl rounded-xl flex items-center justify-center text-red-500 hover:bg-red-500 hover:text-white transition-all border border-red-50"
                    >

    ...
    const exportPDF = () => {
        ...
        const tableRows = sortedPermits.map((permit, index) => {
            ...
            return [
                ...
                qty.toLocaleString(),
                `Rs. ${royaltyBase.toFixed(2)}`,
                `Rs. ${mbl.toFixed(2)}`,
                `Rs. ${gfBase.toFixed(2)}`,
                `Rs. ${totalAmountInRs.toFixed(2)}`,
                `Rs. ${tds.toFixed(2)}`,
                `Rs. ${dmfBase.toFixed(2)}`,
                `Rs. ${gstTotal.toFixed(2)}`,
                `Rs. ${gstPayment.toFixed(2)}`,
                `Rs. ${gstBalance.toFixed(2)}`,
                `Rs. ${rowTotalAmount.toFixed(2)}`,
                `Rs. ${rowTotalPaid.toFixed(2)}`,
                `Rs. ${balancePayment.toFixed(2)}`,
                refs
            ];
        });
        ...
        const totalRow = [
            ...
            `Rs. ${totalRoyalty.toFixed(2)}`,
            `Rs. ${totalMbl.toFixed(2)}`,
            `Rs. ${totalGf.toFixed(2)}`,
            `Rs. ${(totalRoyalty + totalMbl + totalGf).toFixed(2)}`,
            `Rs. ${totalTds.toFixed(2)}`,
            `Rs. ${totalDmf.toFixed(2)}`,
            `Rs. ${(totalGstSeigniorage + totalGstDmf + totalGstGf).toFixed(2)}`,
            `Rs. ${totalGstPayment.toFixed(2)}`,
            `Rs. ${(totalGstSeigniorage + totalGstDmf + totalGstGf - totalGstPayment).toFixed(2)}`,

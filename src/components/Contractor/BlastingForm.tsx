...
const MATERIAL_TYPES = ['Good Boulders', 'Weathered Rocks', 'Soil'] as const;

function getMaterialTamil(type: string): string {
...
function getMaterialAbbr(type: string): string {
  if (type === 'Good Boulders') return 'G.B';
  if (type === 'Weathered Rocks') return 'W.R';
...
export function BlastingForm({ onSuccess }: { onSuccess?: () => void }) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const check = (key: 'pg' | 'ed' | 'edet' | 'nonel_3m' | 'nonel_4m', qty: number, label: string) => {
        let available = 0;
        if (key === 'pg') available = b?.pg?.remaining || 0;
        else if (key === 'ed') available = b?.ed?.remaining || 0;
        else if (key === 'edet') available = b?.edet?.remaining || 0;
        else if (key === 'nonel_3m') available = b?.nonel_3m?.remaining || 0;
        else if (key === 'nonel_4m') available = b?.nonel_4m?.remaining || 0;
    ...
    <form onSubmit={handleSave} className="space-y-4 sm:space-y-6 bg-white rounded-xl sm:rounded-2xl shadow-sm border border-slate-200 p-4 sm:p-6">
      ...
      <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
        ...
        <div className="min-w-0">
          <h3 className="text-base sm:text-lg font-semibold text-slate-900 truncate">{t('New Blasting Record')}</h3>
          <p className="text-xs sm:text-sm text-slate-500 truncate">{t('Record explosives used for blasting')}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:gap-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3">
            {MATERIAL_TYPES.map((type) => (
              ...
              >
                ...
                <div className="text-xs mt-1 opacity-75">{getMaterialTamil(type)}</div>
              </button>
            ))}
          </div>
        </div>
            ...
            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 flex items-center justify-between">
              ...
              {quarryBalances?.ed && (
                <span className={`text-[8px] font-black px-1.5 py-0.5 rounded ${quarryBalances.ed.remaining <= 0 ? 'bg-red-100 text-red-600' : 'bg-emerald-100 text-emerald-600'}`}>
                  {t('Avail:')} {quarryBalances.ed.remaining.toFixed(0)}
                </span>
              )}
            </label>
            <input
            ...
            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 flex items-center justify-between">
              ...
              {quarryBalances?.edet && (
                <span className={`text-[8px] font-black px-1.5 py-0.5 rounded ${quarryBalances.edet.remaining <= 0 ? 'bg-red-100 text-red-600' : 'bg-emerald-100 text-emerald-600'}`}>
                  {t('Avail:')} {quarryBalances.edet.remaining.toFixed(0)}
                </span>
              )}
            </label>
            <input
            ...
            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 flex items-center justify-between">
              ...
              {quarryBalances?.nonel_3m && (
                <span className={`text-[8px] font-black px-1.5 py-0.5 rounded ${quarryBalances.nonel_3m.remaining <= 0 ? 'bg-red-100 text-red-600' : 'bg-emerald-100 text-emerald-600'}`}>
                  {t('Avail:')} {quarryBalances.nonel_3m.remaining.toFixed(0)}
                </span>
              )}
            </label>
            <input
            ...
            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 flex items-center justify-between">
              ...
              {quarryBalances?.nonel_4m && (
                <span className={`text-[8px] font-black px-1.5 py-0.5 rounded ${quarryBalances.nonel_4m.remaining <= 0 ? 'bg-red-100 text-red-600' : 'bg-emerald-100 text-emerald-600'}`}>
                  {t('Avail:')} {quarryBalances.nonel_4m.remaining.toFixed(0)}
                </span>
              )}
            </label>
            <input
          ...
          <label className="block text-xs sm:text-sm font-semibold text-slate-700 mb-2 flex items-center justify-between">
            ...
            {quarryBalances?.pg && (
              <span className={`text-[10px] font-black px-2 py-0.5 rounded ${quarryBalances.pg.remaining <= 0 ? 'bg-red-100 text-red-600' : 'bg-orange-100 text-orange-600'}`}>
                {t('Available:')} {quarryBalances.pg.remaining.toFixed(1)} {t('boxes')} / {(quarryBalances.pg.remaining * PG_BOX_SIZE).toFixed(0)} {t('nos')}
              </span>
            )}
          </label>
          <input
        ...
        <div className="bg-slate-50 p-3 sm:p-4 rounded-lg sm:rounded-2xl border border-slate-100">
          <label className="block text-xs sm:text-sm font-semibold text-slate-700 mb-2 sm:mb-3">
            {t('PG Unit')} <span className="text-red-500">*</span>
          </label>
          <div className="grid grid-cols-2 gap-2 sm:gap-3">
            {PG_UNITS.map((unit) => (
              <button
      ...
      <div className="pt-6 border-t border-slate-200">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h4 className="text-sm font-bold text-slate-900">{t('Monthly Usage Summary')}</h4>
          </div>
          <input 
            type="month" 
            value={selectedMonth}
        ...
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <div className="min-w-[600px]">
              <div className="grid grid-cols-[90px_110px_70px_70px_70px_60px_60px] bg-slate-50 border-b border-slate-200 px-3 py-3.5 font-bold text-slate-500 uppercase text-[10px] tracking-tight text-center">
                <span className="text-left font-black">{t('Date')}</span>
                <span className="text-left font-black">{t('Material')}</span>
                <span>{t('PG (bx)')}</span>
                <span>{t('NLO 3m')}</span>
                <span>{t('NLO 4m')}</span>
                <span>{t('ED')}</span>
              ...
              <div className="divide-y divide-slate-100 max-h-96 overflow-y-auto">
                {statsLoading ? (
                  <div className="p-4 text-center text-xs text-slate-500">{t('Loading summary...')}</div>
                ) : statsError ? (
                  <div className="p-4 text-center text-xs text-red-500">{statsError}</div>
                ) : monthlyStats.length === 0 ? (
                  <div className="p-4 text-center text-xs text-slate-500 italic">{t('No records for ')}{safeFormat(selectedMonth + '-01', 'MMMM yyyy')}</div>
                ) : (
                    ...
                    <div key={`${stat.date}-${stat.material_type}-${idx}`} className="grid grid-cols-[90px_110px_70px_70px_70px_60px_60px] px-3 py-4 items-center hover:bg-slate-50/50 transition-colors text-center text-xs">
                      ...
                      <div className="text-left">
                        <div className="font-bold text-orange-600 truncate mr-1">
                          {getMaterialAbbr(stat.material_type)}
                        </div>
                        <div className="text-[9px] text-slate-400">
                          {getMaterialTamil(stat.material_type)}
                        </div>
                      </div>
          ...
          {monthlyStats.length > 0 && (
            <div className="bg-slate-900 p-4 text-white">
              <h5 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4 border-b border-white/10 pb-2">{t('Total Monthly Usage')}</h5>
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                <div className="bg-white/5 p-2 rounded-lg border border-white/10 flex flex-col items-center">
                  ...
                  <span className="text-sm font-black text-white">
                    ...
                    <span className="text-[8px] opacity-40 ml-0.5 font-normal capitalize">{t('nos')}</span>
                  </span>
                </div>

                <div className="bg-white/5 p-2 rounded-lg border border-white/10 flex flex-col items-center">
                  <span className="text-[8px] text-slate-500 uppercase font-bold tracking-wider mb-1">{t('EDET')}</span>
                  <span className="text-sm font-black text-white">
                ...
                <div className="bg-white/5 p-2 rounded-lg border border-white/10 flex flex-col items-center">
                  <span className="text-[8px] text-slate-500 uppercase font-bold tracking-wider mb-1 text-center">{t('NLO 3m')}</span>
                  <span className="text-sm font-black text-white">
                ...
                <div className="bg-white/5 p-2 rounded-lg border border-white/10 flex flex-col items-center">
                  <span className="text-[8px] text-slate-500 uppercase font-bold tracking-wider mb-1 text-center">{t('NLO 4m')}</span>
                  <span className="text-sm font-black text-white">
                ...
                <div className="bg-orange-600/20 p-2 rounded-lg border border-orange-600/30 flex flex-col items-center col-span-2 sm:col-span-1">
                  <span className="text-[8px] text-orange-400 uppercase font-bold tracking-wider mb-1">{t('PG Total')}</span>
                  <span className="text-sm font-black text-orange-400">
                    {monthlyStats.reduce((acc, curr) => acc + curr.pg, 0).toFixed(2)}
                    <span className="text-[8px] opacity-60 ml-0.5 font-normal capitalize text-orange-400">{t('bx')}</span>
                  </span>
                </div>
              </div>
            </div>
